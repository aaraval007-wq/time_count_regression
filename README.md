...
| `.aic_bic_comparison()` | None | Fits the model with and without each active feature. Reports ΔAIC and ΔBIC. Negative ΔAIC means dropping the feature improves fit. |
...
| `PoissonWrapper` | Poisson GLM | Poisson PMF. Best when variance ≈ mean. |
...
| `.dispersion_check(model)` | `ModelWrapper` | Pearson chi² / df on out-of-sample predictions. Most meaningful for Poisson. |
